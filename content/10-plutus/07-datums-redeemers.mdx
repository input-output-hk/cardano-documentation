---
title: Working with datums and redeemers
metaTitle: Working with datums and redeemers
---

Two important elements in Plutus are datums and redeemers, here we learn what these are and how to work with these elements when submitting transactions.
...
The following concepts are important to understand in the context of a redeemer transaction:
...
She wants to lock 20 ADAs to the “datum-redeemer” script. She can do that by sending a transaction attaching an arbitrary data (i.e. the number 42) to the new UTXO:
...
1. Lock a UTXO in a script address by attaching a hash of the datum to the UTXO.
2. Unlock the funds with a redeemer that corresponds to the datum. 

This is expressed in a contract, as follows:
...
### Prerequisites
The Plutus script needs to be built with GHC 8.10.4. One way to install this specific version is to use [ghcup](https://gitlab.haskell.org/haskell/ghcup-hs). Alternatively, you can use Nix. 

### Compiling the contract
In this tutorial, we are going to modify one of the examples of the [Alonzo testnet repository](https://github.com/input-output-hk/Alonzo-testnet). Follow these steps:
