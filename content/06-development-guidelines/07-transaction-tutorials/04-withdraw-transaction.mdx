...
```
In this case you have ~29 ADA. Then you need some of your own ADA to pay the fees, so you need to query your payment address:
```
...
```
Notice that this transaction will take funds from the stake address, so it will need to be signed with the `stake.skey` file too.
```
...
Finally, verify that you have indeed received your rewards in your payment address:
```
$ cardano-cli query utxo --address $(cat payment.addr)
                           TxHash                                 TxIx        Amount
--------------------------------------------------------------------------------------
