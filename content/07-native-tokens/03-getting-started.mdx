...
cardano-node version 1.35.0 and later of the cli no longer supports ASCII token names, only hex-encoded format is supported. The result of running  
...
- `${assetName}` (where assetName is hex-encoded assetName)
...
- multi-asset values can be defined as:
  - `INT policyid.assetName`, e.g. `42 $MYPOLICY.assetName`
  - `INT policyid`, e.g. `42 $MYPOLICY` (No asset name specified)
  - `policyid.assetName`, e.g `$MYPOLICY.assetName` (This will mint only one of `myassetname`)
