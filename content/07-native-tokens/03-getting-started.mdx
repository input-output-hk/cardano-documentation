...
The latest configuration files can be found [here](https://hydra.iohk.io/job/Cardano/cardano-node/cardano-deployment/latest-finished/download/1/index.html). You will need to download all the files that are associated with the `testnet` cluster and copy them to your `config` subdirectory.
...
cardano-node version 1.35.0 and later of the cli no longer supports ASCII token names, only hexidecimal format is support. The result of running  $ echo -n "assetName" | xxd -ps
...
- `${quantity} ${policyId}.${assetName(hexidecimal format)}`
- `${quantity} ${policyId}` 
...
- multi-asset values can be defined as:
  - `INT policyid.assetName(hexidecimal format)`, e.g. `42 $MYPOLICY.assetName(hexidecimal format)`
  - `INT policyid`, e.g. `42 $MYPOLICY` (No asset name specified)
  - `policyid.assetName(hexidecimal format)`, e.g `$MYPOLICY.assetName(hexidecimal format)` (This will mint only one of `myassetname`)
- Multiple assets can be combined in the same multi-asset value using the `+` operator, e.g:

`100 lovelace + 42 $MYPOLICY.foo(hexidecimal format) + -2 $MYPOLICY.bar(hexidecimal format) + 10 lovelace`
...
- `-42 $MYPOLICY`
- `-72191 $MYPOLICY.foo(hexidecimal format)`
- `-100`
- `-920 lovelace`
...
- `20 $MYPOLICY + 12 $MYPOLICY.foo(hexidecimal format) + -2 $MYPOLICY.bar(hexidecimal format)`
- `201 4$MYPOLICY.foo(hexidecimal format) + 12 + -1 + 9 lovelace + 10 $MYPOLICY`
...
```
cardano-cli query utxo --testnet-magic
```
...
```
cardano-cli query utxo --address "$ADDRESS" --testnet-magic
```
...
1. Download the node and config files for the public testnet using this code:

```bash
wget https://hydra.iohk.io/build/16159630/download/1/cardano-node-1.35.0-linux.tar.gz
tar xzvf cardano-node-1.35.0-linux.tar.gz
...
wget https://hydra.iohk.io/build/13695229/download/1/testnet-topology.json
cd ..
```
...
```bash
./cardano-cli query utxo --address addr_test1vqvlku0ytscqg32rpv660uu4sgxlje25s5xrpz7zjqsva3c8pfckz --testnet-magic 1097911063
```
...
```
$echo -n "melcoin" | xxd -ps => 6d656c636f696e
...
```bash
./cardano-cli query utxo --address addr_test1vp8s8zu6mr73nvlsjf935k0a38n8xvp3fptkyz2vl8pserqkcx5yz --testnet-magic 1097911063
...
```
cardano-cli transaction submit --tx-file  "$TX_FILE" --testnet-magic 1097911063
```
...
cardano-cli transaction submit --tx-file "$TX_FILE_1" --testnet-magic 1097911063
```
