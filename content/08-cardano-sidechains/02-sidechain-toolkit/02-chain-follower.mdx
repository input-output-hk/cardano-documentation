...
After installing and setting up a DB Sync instance, you need to configure a Mamba node to point to it. In the node’s configuration file set the datasource type to `db-sync` and configure connection details to match your setup. The finished configuration should look similar to the example below:
