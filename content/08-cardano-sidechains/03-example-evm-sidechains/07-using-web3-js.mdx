...
This section helps learning how to use the web3 Javascript interface with the Cardano EVM sidechain client for application development on the testnet. You can also walk through an example program that demonstrates some sample API calls.  
...
Follow the instructions on the [node authentication page](../network/node-authentication). You will receive a one-time access key that you must use within 24 hours. This process will authorize you to connect the machine you are currently using to the RPC URL (`http://faucet.mamba.atago/`). It's the same URL as the faucet.
