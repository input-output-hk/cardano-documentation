...
In this tutorial, you will use MetaMask as your wallet and Web3 provider. If you have not done so already, [create or import an account with MetaMask](../01-metamask.mdx) and fund your wallet with testnet tokens using the [faucet](<PENDING_URL>).
...
- Search the transaction hash or contract address on the EVM sidechain [testnet explorer](<PENDING_URL>).
...
🎉 Hurray! You deployed your smart contract to EVM sidechain testnet and can search your deployment on the [testnet explorer]( <PENDING_URL>).
...
Upon executing `truffle migrate`, Truffle will return the transaction hash and contract address which can be searched on the EVM sidechain [testnet explorer](<PENDING_URL>).
