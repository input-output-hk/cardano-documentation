...
1. We will use the **sidechain-cli** and **sc-evm-cli** within this tutorial.
This tutorial will use the **sidechain-cli** and **sc-evm-cli** components. Download the binaries and add them to your shell’s path: https://github.com/input-output-hk/sidechains-tooling.
...
🎉 Congradulations! At this point, you have successfully claimed your `SC_Token` on the mainchain.

# Transfering tokens from mainchain to sidechain

### Step 01 - Burn SC_Token

To transfer `SC_Token` from mainchain to sidechain, use the **sidechain-cli** to burn mainchain `SC_Token`.
...
- `--signing-key-file payment.skey`: GGenerating a mainchain account creates a `.skey` file to sign transactions. The command gives the path to this file.
...
🎉 Congradulations! At this point, you have successfully burned mainchain `SC_Token` for native sidechain `SC_Token`.
