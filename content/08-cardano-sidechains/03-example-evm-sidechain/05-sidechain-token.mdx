...
Test Ada (`tAda`) is the native token of the *dedicated* Cardano testnet (seperate from Preview and Preprod testnets). `tAda` is required to pay for transaction fees conducted on the Cardano testnet. `tADa` is carries no real world value.
