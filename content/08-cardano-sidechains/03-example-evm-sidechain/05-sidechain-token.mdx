...
Test Ada (`tAda`) is the native token of the *dedicated* Cardano testnet (separate from the Preview and Preprod testnets). `tAda` is required to pay for transaction fees conducted on the Cardano testnet. `tADa` carries no real world value.
