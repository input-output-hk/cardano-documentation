...
1. **sidechain-evm-cli** is used within this tutorial and is in the SC_EVM repository [here](https://github.com/input-output-hk/sc-evm/tree/main/tooling).
...
4. The bridge contract source which can be used to compile Solidity or obtain the ABI is in the SC_EVM repository [here](https://github.com/input-output-hk/sc-evm/blob/main/src/solidity-bridge/contracts/Bridge.sol).
...
### Step 01 - Set up contract interface in RemixIDE

- Go to https://remix.ethereum.org/

- Create `/contracts/bridge.abi` with the following contents: [bridge.abi](https://github.com/input-output-hk/sc-evm/blob/main/src/solidity-bridge/contracts/Bridge.sol)
...
### Step 03 - Wait for the transaction to be processed and obtain the Merkle proof
...
- Check `SC_Token` token balance using the JSON-RPC API: 
