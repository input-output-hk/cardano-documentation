...
[Voltaire](https://roadmap.cardano.org/en/voltaire/) voting mechanism. 
...
Vasil is the protocol upgrade, which will be implemented in June 2022. Named after the late Bulgarian mathematician and prominent Cardano community member Vasil Dabov, the Vasil upgrade introduces five key mechanisms to improve the blockchain's performance: [CIP-31](https://cips.cardano.org/cips/cip31/) (Reference Inputs), [CIP-32](https://cips.cardano.org/cips/cip32/) (Inline Datums), [CIP-33](https://cips.cardano.org/cips/cip33/) (Reference Scripts ), CIP-40 (Collateral Outputs), and diffusion pipelining.

These improvements boost Cardano's usability and scalability by, for example, increasing the block size limit to fit more transactions per block. Developers will have a better experience while building on Cardano as Vasil greatly reduces the complexity of creating and deploying DApps on Cardano.

Plutus scripts are also the focus of Vasil. These scripts will now live persistently on-chain so they can be referenced when needed. This is good because it improves efficiency, and there’s no longer a need to include the script in the transaction attempting to spend its outputs.
