...
*Mary* is the Shelley protocol upgrade implemented in March 2021. It introduces native token and multi-asset support on Cardano. Mary allows users to create uniquely defined (custom) tokens and carry out transactions with them directly on the Cardano blockchain.

With Mary upgrade, the ledger’s accounting infrastructure processes not only ada transactions but also transactions that simultaneously carry several asset types. Native support grants distinct advantages for developers as there is no need to create smart contracts to handle custom token creation or transactions. This means that the accounting ledger tracks the ownership and transfer of assets instead, removing extra complexity and potential for manual errors, while ensuring significant cost efficiency.
...
*Alonzo* is the next protocol upgrade, which marks Goguen development theme. It builds on top of transaction metadata, token locking, and native asset functionality to allow for functional smart contract development.
