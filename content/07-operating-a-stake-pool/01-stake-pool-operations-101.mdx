...
Cardano node v.8.7.3 addresses a minor fix in the `ouroboros-network`. It resolves an issue in the `outbound-governor` where, under rare but reproducible conditions, it could lose track of certain outbound connections, such as a local root or another node.
