...
When a contract runs, the roles involved are fulfilled by participants, which are identities on the blockchain. Each role is represented by a token on the chain, and roles can be transferred during contract execution (be traded, in other words). 
