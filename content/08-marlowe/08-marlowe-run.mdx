...
**Note**: This prototype version of Marlowe Run provides a demo version that uses test contracts and test funds. The full version of Marlowe Run will be available soon.

## Accessing the Marlowe Run Demo
To access and view the dashboard, follow these steps:
1. Visit the Marlowe website and click **Marlowe Run** on the main menu.
2. Click **Try demo**.
3. The Marlowe Run demo is launched:

![demo-launch](demo-launch.png)

 For this demo version of Marlowe Run, test wallets with 10k ADA in them are generated so that you can use the service. You can use any wallet that exists on the server as  long as you know the public key. This will become the integration point with a real wallet in the full version of Marlowe Run, where only the wallet owner can authorize  payments.
...
9. Click **Setup**, or **Back** to return to the contract template selection card.
10. Enter the following details for the contract:
    - title
    - role information: buyer, mediator, seller
...
13. Then click Pay and start. The contract is now running.

![pay-start](pay-start.png)

The running contract dialog is displayed on the contracts dashboard. This shows a counter with the time allocated within which the seller must make a deposit.

14. Now the buyer must make their deposit. 
15. Once they make their deposit, you have two choices:
    - Click Everything is alright to confirm you are happy to proceed.
    - Click Report problem if there was an issue with the contract.
...
You can click **Close contract** at any time to close the running contract.
