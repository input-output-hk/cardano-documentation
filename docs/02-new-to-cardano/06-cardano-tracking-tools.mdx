...
> Note: IOHK has developed a stake pool metadata aggregation server
> [(SMASH)](/getting-started/operating-a-stake-pool/SMASH) to provide the
> community with a list of verified stake pools with valid metadata. Smash is
> integrated with the Daedalus wallet, and users can see a list of valid stake
> pools in the delegation center tab.
