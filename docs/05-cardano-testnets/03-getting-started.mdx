...
need [to get some test ada](/cardano-testnet/tools/faucet) to test your
transactions.
...
When installing the node, it is also essential to build the
[cardano-client](https://github.com/input-output-hk/cardano-node/tree/master/cardano-client-demo).
This tool is built similarly to the node. Since it is so fundamental, all
tutorials that build the node also build the client.

## Configurations

To start working with the node, it is essential to ensure that the node
configuration, genesis and topology files are correctly set up. In addition, the
node will need the configuration files specific to each era that the Cardano
