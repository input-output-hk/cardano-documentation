...
The native tokens feature extends the accounting infrastructure defined
...
This feature accommodates transactions that simultaneously use a range of assets. These assets
...
must hold a [minimum ada value](https://cardano-ledger.readthedocs.io/en/latest/explanations/min-utxo-alonzo.html) (`min-ada-value`).
...
2.  A specification of during what slot the script can be spent from (eg,
...
Note that this is not as restrictive as it sounds. In a loose parallel with the US
...
with the multi-signature or single issuer policy. For example, this output can be spent
