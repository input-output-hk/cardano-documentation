...
Regular monitoring of active nodes is crucial to determine if the node is
running optimally, if the system is performant, or if fine-tuning is required.

Activate a trace on the node to access metrics, including network traffic,
memory usage, and general blockchain data. For details, see:
